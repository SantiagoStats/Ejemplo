...
pie(Aglomerado.frec,
    labels = c("CABA","GBA"),
    col=c("red","blue"),
    main="Gráfico circular para la variable Aglomerado")
...
barplot(Aglomerado.frec,
        col=c("red","blue"),
        main="Gráfico de barras para la variable Aglomerado")
...
#tapply aplica una función a un vector en los subvectores que define otro vector máscara
#en este caso calcula el ingreso medio, por region

100*tapply(Ingresos, Región, sd)/tapply(Ingresos, Región, mean)

#Que hace si le aplico una función a todo el data.frame?
summary(DatosEPH)
...
hist(Ejemplos$N, 
     ...
     main=" ",
     xla="",
     sub="Distribución Simétrica")

hist(Ejemplos$AS, 
     col="blue",
     main=" ",
     xla="",
     sub="Distribución Asimétrica a Derecha")
